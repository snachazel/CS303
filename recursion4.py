  ...
  if start == len(nums):
    ...
    for i in range( start + 1 , len(nums)):
      if nums[start] == nums[i]:
        sameSum += nums[i]
        count += 1 
    return groupSumClump(start + count , nums , target - sameSum) or groupSumClump(start + count , nums , target)
...
def splitArray(nums):
  subArray = []
  return( splitArrayHelper( nums , subArray , 0))


def splitArrayHelper(a , subArray , start):
  if start == len(a):
    return sum(subArray) == (sum(a) - sum(subArray))
  else:
    c = subArray[:]
    subArray.append(a[start])
    return (splitArrayHelper( a , subArray , start + 1) or splitArrayHelper( a , c , start+ 1))
...
def splitOdd10(nums):
  subArray = []
  return splitOdd10Helper(nums, subArray , 0)


  
def splitOdd10Helper(a , subArray , start):
  if start == len(a):
    return sum(subArray) % 10 == 0 and ((sum(a) - sum( subArray)) % 2 == 1)
  else:
    c = subArray[:]
    subArray.append(a[start])
    return (splitOdd10Helper( a , subArray , start + 1) or splitOdd10Helper( a , c , start+ 1))
...
def split53(nums):
  subArray = []
  return split53Helper( nums , subArray , 0)

def split53Helper( a , subArray , start):
  if start == len(a):
    return sum(subArray) == (sum(a) - sum(subArray))
  else:
    if a[start] % 5 == 0:
      c = subArray[:]
      subArray.append(a[start])
      return split53Helper(a , subArray , start + 1) or split53Helper(a , c, start + 1)
    if a[start] % 3 == 0 and a[start] % 5 != 0:
      return split53Helper(a , subArray , start + 1)
